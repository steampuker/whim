...
*/
#include "shader.c"    /* <-- Compiled shader code is here. */
...
int main(int argc, char** argv)
{
    ...
    */
    if(!whimInit(0)) {
        printf("Failed to initialize whim!\n");
        return -1;
    }

    WhimWin *window = whimWinCreate(&(WhimRect){0, 0, 640, 480}, "Vulkan Tutorial", WHIM_HEX_TO_COLOR(0), 0);
    if(!window) {
        printf("Failed to create whim window!\n");
        whimDeinit();
        return -1;
    }
    ...
    */
    ...
    ppEnabledExtensionNames[enabledExtensionCount++] = VK_KHR_XCB_SURFACE_EXTENSION_NAME;
#endif
    ...
    */
    ...
    surfaceInfo.pNext  = NULL;
    surfaceInfo.flags  = 0;
    surfaceInfo.connection = x11.hook.connection;
    surfaceInfo.window = window->window_id;
    result = vkCreateXcbSurfaceKHR(instance, &surfaceInfo, NULL, &surface);
    if (result != VK_SUCCESS) {
        vkDestroyInstance(instance, NULL);
        whimWinDestroy(window);
        whimDeinit();
        printf("Failed to create a Vulkan surface for the main window.");
        return -1;
    }
    ...
    */
    ...
    while (1) {
        WhimEvent event;
        whimPollEvents(&event);
        if(event.type == WHIM_EVENT_CLOSE)
            break;
        else if(event.type == WHIM_EVENT_KEY) {
            printf("Key event: %d, is pressed: %s\n", event.as_key.keycode, event.as_key.is_pressed ? "true" : "false");
        }
